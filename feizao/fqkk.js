...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    let acList = fqkk.filter(o => o.hd).map((o, i) => ({no: i+1, uid: o.uid, gold: -1, rest: -1, num: -1, url: o.url, headers: JSON.parse(o.hd)}));
    let execAcList = [];
    ...
    for (let arr of execAcList) {
      ...
      $.msg(`${$.name} ${allAc}`, '', rtList.map(ac => `【账号${ac.no}】余额：${ac.gold}币 阅读数：${ac.num} 待读数：${ac.rest}${ac.msg?'\n'+ac.msg:''}`).join('\n'));
    }
  }
})()
.catch((e) => $.logErr(e))
      ...
      try {
        if (err) {
          ...
          if (result.data && result.data.infoView) {
            ...
            ac.gold = (result.data.infoView.score || 0) - 0;
      ...
      $.post(opts, async (err, resp, data) => {
        ...
        try {
          ...
          if (result.code == 0 && result.data && result.data.jkey) {
            ...
            if (jumpObj) {
              $.log(`🌝账号${ac.no}等待10秒后提交本次阅读, jump接口结果：\n${JSON.stringify(jumpObj, null, 2)}`);
              await $.wait(10000);
